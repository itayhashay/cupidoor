...
const UploadsForm = ({apartmentData, saveImages, uploadedImages} : {apartmentData: StepperApartment,  saveImages: (files: UploadedImage[]) => void, uploadedImages: UploadedImage[]}) => {
    ...
    //     apartmentData._id && fetchApartmentImages(apartmentData._id)
    //         .then(async(images: ServerApartmentImages[]) => {
    //             const files: Promise<File[]> = Promise.all(
    //                 images.map(async (image: ServerApartmentImages) => {
    //                     console.log(image.url)

    //                 const file: File =  await getImageAsFile(image.url);
    //                 console.log(file)
    //                 return file;
    //             }));

    //             files.then((files: File[]) => setFiles(files))  
    //         })
    // }, [apartmentData]);
    ...
    const handleFileRemove = (error: FilePondErrorDescription | null, removedFile: FilePondFile) => {
        ...
        !isNewFile(removedFile.file as File) && setRemovedImages((prevFiles) => [...prevFiles, removedFile.file.name]);
    };
